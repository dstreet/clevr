...
test('`get` method could call the model\'s default query', async () => {
	const fn = jest.fn()
	const Data = ModelBuilder
		.create()
		.addQuery('default', fn)
...
test('`applyMutation` method could call the model\'s `applyMutation`', async () => {
	const fn = jest.fn()
	const Data = ModelBuilder
		.create()
		.addMutation('foo', fn)
...
test('`create` method could call the model\'s `initializer`', async () => {
	const fn = jest.fn()
	const Data = ModelBuilder
		.create()
		.setInitializer(fn)
...
test('`remove` method could call the model\'s `destructor`', async () => {
	const fn = jest.fn()
	const Data = ModelBuilder
		.create()
		.setDestructor(fn)
