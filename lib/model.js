...
module.exports = class Model extends EventEmitter {
	...
	async get(...input) {
		return this.query.apply(this, ['default'].concat(input))
	}
	...
	async query(name, ...input) {
		...
		const sources = await query.apply(this, input)
	...
	async create(inputData) {
		...
		let initializerData = inputData

		if (typeof inputData === 'object') {
			initializerData = Object.assign({},
		...
		for (const binding of this._bindings.create) {
			if (binding.fn(document, binding.input)) {
				this.emit(binding.name, document)
			}
		}
	...
	bind(name, input) {
		...
		this._bindings[binding.type].push({ name: binding.name, fn: binding.fn, input })
