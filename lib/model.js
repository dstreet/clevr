...
module.exports =  class Model {
	...
	async query(name, input) {
		...
		if (Array.isArray(sources)) {
			return sources.map(item => {
				const data = this._transformSources(item)

				return {
					selector: this._definition.documentSelector(data, item),
					data
				}
			})
		} else {
	...
	async applyMutation(name, document, data) {
		...
		const sources = await mutation.fn(document, data)
		const newData = this._transformSources(sources)

		return [{
			selector: this._definition.documentSelector(newData, sources),
			data: newData
		}]
	}
