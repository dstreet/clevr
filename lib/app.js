...
class App extends EventEmitter {
	constructor(namespace, isMaster, networkOpts) {
		...
		if (networkOpts && !networkOpts.localOnly) {
			let key, cert
			...
			const remoteServer = new Server(namespace, this.networkOpts)

			remoteServer.on('error', err => this.emit('error', err))
			remoteServer.on('connection_reset', () => this.emit('connection_reset'))

			this.server.addServer(remoteServer)
		}
